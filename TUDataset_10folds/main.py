...
parser.add_argument("--run_times", type=int, default=2, help="seed for initializing training.")
...
def main(dataset):
    ...
    fold_path = os.path.join('datasets', args.dataset, '10fold_idx')
    ...
    for combination in combinations:
        ...
        for run in range(args.run_times):
            ...
            for fold_number in range(1, 11):
                ...
                train_idx = torch.as_tensor(np.loadtxt(
                    os.path.join(fold_path, 'train_idx-{}.txt'.format(fold_number)), dtype=np.int32), dtype=torch.long)
                test_idx = torch.as_tensor(np.loadtxt(
                    os.path.join(fold_path, 'test_idx-{}.txt'.format(fold_number)), dtype=np.int32), dtype=torch.long)
                val_idx = torch.as_tensor(np.loadtxt(
                    os.path.join(fold_path, 'test_idx-{}.txt'.format(val_fold_number)), dtype=np.int32), dtype=torch.long)
                # val_idx = train_idx
...
if __name__ == '__main__':
    datasets = ['DD', 'PROTEINS', 'MUTAG', 'IMDB-BINARY', 'IMDB-MULTI', 'COLLAB']
    datasets = ['PROTEINS']
