...
class MatPool():
    def __init__(self, args):
        ...
        self.lr = args.lr
        self.optimizer = torch.optim.Adam(self.net.parameters(), self.lr, weight_decay=args.weight_decay)

        num_params = sum(p.numel() for p in self.net.parameters())
        print(f'#Params: {num_params}')

        # molbbbp pos_weight=torch.tensor([0.01]), reduction='sum' moltoxcast moltox21 reduction='sum'
        if args.dataset == 'ogbg-molbbbp':
            self.cls_criterion = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([0.01]),
                                                            reduction='sum').to(self.device)
        else:
            self.cls_criterion = torch.nn.BCEWithLogitsLoss(reduction='sum')

        if args.dataset == 'ogbg-molhiv':
            self.cls_criterion = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([100]),
                                                            reduction='sum').to(self.device)

        # self.cls_criterion = torch.nn.BCEWithLogitsLoss().to(self.device)
    ...
    def fit(self, train_loader, valid_loader, epochs, task_type):
        ...
        for epoch in range(epochs):
            ...
            train_score = self.evaluator.eval(input_dict)['rocauc']
            valid_score = self.predict(valid_loader)['rocauc']
