...
### PGCN convolution along the graph structure
    ...
    def __init__(self, args, node_encoder):
        ...
        for i in range(self.num_layer):
            if args.gnn == 'gin':
                ...
                self.convs.append(PGCNConv(emb_dim=self.emb_dim, use_edge_attr=self.use_edge_attr))
            elif args.gnn == 'pem':
        ...
        # self.net = Network(num_layer=self.num_layer, emb_dim=self.emb_dim, num_classes=self.num_tasks)
        self.net = Network(num_layer=1, emb_dim=self.emb_dim, num_classes=self.num_tasks)
...
class Network(torch.nn.Module):
    def __init__(self, num_layer=3, emb_dim=256, num_classes=10, max_seq_len=5):
        ...
        self.mlp = nn.Sequential(
            ...
            MatNet(channels=num_layer, left_in=emb_dim // 4, left_out=emb_dim // 16, right_in=emb_dim // 4,
                   right_out=emb_dim // 16),
            nn.BatchNorm2d(num_layer),
        )
        self.graph_pred_linear_list = torch.nn.ModuleList()
        for i in range(max_seq_len):
            self.graph_pred_linear_list.append(torch.nn.Linear(emb_dim // 16 * emb_dim // 16 * num_layer, self.num_classes))
