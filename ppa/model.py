        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
                ...
                if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
                    ...
                    loss = self.multicls_criterion(pred.to(torch.float32), batch.y.view(-1,))
                    loss.backward()
                    self.optimizer.step()
            ...
            train_score = self.evaluator.eval(input_dict)['acc']
            valid_score = self.predict(valid_loader)['acc']
